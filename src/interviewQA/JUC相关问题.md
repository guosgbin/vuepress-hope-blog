...
1. **内核线程的局限**：1:1 的内核线程模型是如今 Java 虚拟机的主流选择，但是这种映射到操作系统上的线程的缺陷是**切换、调度成本高昂，而且系统能容纳的线程数量也有限**；
...
指的是当线程池中的线程数量超过核心线程数时，多余的空闲线程在被销毁之前等待新任务的最长时间。（当前核心线程也可以设置超时）
...
3. 在线程工厂 ThreadFactory 中设置  UncaughtExceptionHandler。
...
- **RUNNING**：能够接收新任务，同时也会处理任务队列里的任务。
- **SHUTDOWN**：不接受新的任务，但是会去处理队列中的任务。
- **STOP**：既不会接收新任务，也不会处理任务队列里的任务，并且会尝试中断正在执行的任务。
- **TIDYING**：当线程池中的所有任务都已终止，并且工作线程数量为 0 时，线程池会进入 `TIDYING` 状态。此时，会调用 `terminated()` 钩子方法。
- **TERMINATED**：在 `terminated()` 方法执行完毕之后，线程池就进入了 `TERMINATED` 状态，这意味着线程池的生命周期结束。

为了给这些个状态作比较，也就是判断数值的大小，所以它们的数值大小很重要，运行状态随时间增加，但不保证能够到达每个状态。

**线程池的状态转换过程**：

-  `RUNNING -> SHUTDOWN`：**调用了 shutdown() 方法，该方法还隐藏在 finalize() 方法中。**线程池停止接收新任务，但会继续处理队列中的任务。 

-  `(RUNNING or SHUTDOWN) -> STOP`：**调用了 shutdownNow() 方法。** 线程池停止接收新任务，不再处理队列中的任务，并尝试中断正在执行的任务。

-  `SHUTDOWN -> TIDYING`：当任务队列和工作线程都为空时，线程池从 `SHUTDOWN` 状态进入 `TIDYING` 状态。

-  `STOP -> TIDYING`：当工作线程都停止时，线程池从 `STOP` 状态进入 `TIDYING` 状态。 

-  `TIDYING -> TERMINATED`：`terminated()` 方法执行完毕后，线程池进入 `TERMINATED` 状态。
...
- getActiveCount()：获取线程池中活跃线程数（正在执行任务的线程数量）；（近似值，执行任务加锁的原因）
...
<font color='blue'>线程活跃度（活跃线程数）</font>：

- **活跃线程数经常达到最大线程数**：
  - 分析：表明任务量较大，也可能短时间内有大量任务提交到线程池。
    - 核心线程数可能设置过小，
    - 或者任务队列容量不足，导致需要频繁创建新线程。
    - 或者消费速度过慢。
  - 优化：可增加核心线程数或最大线程数，同时根据实际情况调整任务队列大小。
- **活跃线程数长时间为 0**：
  - 分析：意味着线程池处于空闲状态，可能存在资源浪费，可考虑缩小线程池规模。
  - 优化：可通过调整线程池的创建和销毁策略，如设置合适的线程空闲时间，让线程在空闲一段时间后自动销毁。

<font color='blue'>任务队列长度</font>：

- **任务队列经常为空**：
  - 分析：表示任务处理速度较快，线程资源可能有闲置，可考虑减少线程数。
  - 优化：可适当减少核心线程数，降低系统资源消耗。
- **任务队列经常满**：
  - 分析：说明任务生产速度大于消费速度，可能需要增加线程数或扩大任务队列容量。
  - 优化：
    - 可增加核心线程数或最大线程数，提高任务处理速度；
    - 若线程数已经达到上限，可考虑扩大任务队列容量；
    - 也需要考虑优化任务消费速度。
- **任务队列短时间堆积**：
  - 分析：可能是突发流量导致的。
  - 优化：可以考虑增大最大线程数，增加线程存活时间。

<font color='blue'>任务执行时间</font>：

- **任务执行时间普遍较短**：但任务队列仍有积压
  - 分析：可能是线程数不足或任务调度不合理。
  - 优化：可增加线程数
- **部分任务执行时间过长**：
  - 分析：会导致其他任务等待时间增加，影响整体性能，需要找出这些慢任务并进行优化。
  - 优化：对于执行时间过长的慢任务，可分析其具体原因，如是否存在 I/O 阻塞、数据库查询缓慢等问题，并进行针对性优化。

<font color='blue'>任务拒绝率</font>：

- **任务拒绝率较高：**
  - 分析：说明线程池无法及时处理所有任务，可能是线程数不足、任务队列容量过小或任务提交速度过快。
  - 优化：
    - 增加核心线程数或最大线程数，提高线程池的处理能力。
    - 扩大任务队列容量，让更多任务能够在队列中等待处理。
    - 优化任务的处理速度。
...
**线程池使用一张 Hash 表去持有线程的引用**，线程池可以方便地添加、移除和管理线程，实现对线程生命周期的控制。当创建新的 `Worker`（也就是新线程）时，会将其添加到 `workers` 集合中；当需要回收线程时，会从该集合中移除对应的 `Worker` 引用。
...
- **快速处理一些批处理任务**。比如批量文件上传、批量数据导入、批量图片处理等，线程池能够将大任务拆分成多个小任务，然后并发执行这些小任务。

### 线程池异步和 MQ 适用场景有啥不同

- **线程池**：适用于**处理大量的短时间、高并发的任务，需要快速响应和处理的场景**。例如，Web 服务器处理客户端请求、数据库连接池管理、异步 I/O 操作等。线程池可以提高系统的并发性能和资源利用率，减少线程创建和销毁的开销。
- **消息队列**：适用于需要**解耦系统组件、实现异步处理、削峰填谷和保证消息可靠传输的场景**。例如，电商系统中的订单处理、物流跟踪，分布式系统中的服务间通信，日志收集和处理等。消息队列可以提高系统的可扩展性和稳定性，降低系统的耦合度。
...
graph LR
    ...
    A([开始]):::startend --> B(创建调度线程池):::process
    B --> C(定义任务):::process
    C --> D{选择任务类型}:::decision
    D -->|一次性任务| E(调用 schedule 方法):::process
    D -->|固定速率周期性任务| F(调用 scheduleAtFixedRate 方法):::process
    D -->|固定延迟周期性任务| G(调用 scheduleWithFixedDelay 方法):::process
    E --> H(任务包装成 ScheduledFutureTask):::process
    F --> H
    G --> H
    H --> I(计算触发时间):::process
    I --> J(将任务添加到 DelayedWorkQueue):::process
    J --> K{任务是否到期}:::decision
    K -->|是| L(工作线程从队列取出任务执行):::process
    K -->|否| M(线程阻塞等待):::process
    M --> K
    L --> N{是否为周期性任务}:::decision
    N -->|是| O(重新计算触发时间):::process
    O --> J
    N -->|否| P(任务执行结束):::process
    P --> Q{是否关闭线程池}:::decision
    Q -->|是| R(调用 shutdown 或 shutdownNow 方法):::process
    R --> S([结束]):::startend
    Q -->|否| D
```
...
- **long period**：任务的执行周期 
  - period > 0：表示固定速率的周期任务，scheduleAtFixedRate 提交的，不考虑任务的执行时间；
  - period < 0：表示固定延迟的周期任务，scheduleWithFixedDelay 提交的，考虑任务的执行时间；
  - period = 0：表示执行一次的延迟任务；
...
在 `DelayedWorkQueue` 中，任务的优先级由其触发时间决定，触发时间早的任务优先级高，会先被执行。
...
- 使用**数组来表示这个满二叉树**：
  ...
  - 查询右子节点：i * 2 + 2
- **插入元素的流程**：新元素会被添加到数组的末尾，然后通过上浮操作将其调整到合适的位置，以保证堆的性质。
  - 执行**向上冒泡**的逻辑，首先会将要插入节点放到数组的最后一个节点的后面，然后依次和父节点做比较，假如当前节点的优先级比父节点的高，就交换它们的位置。直到当前节点比父节点的优先级低或者已经冒泡到了根节点了；

- **删除元素的流程**：堆顶元素被移除后，会将数组末尾的元素移动到堆顶，然后通过下沉操作将其调整到合适的位置。
  - 执行**向下冒泡**的逻辑，将数组中最后一个节点和根节点的左右节点比较，假如最后一个节点的优先级要比左右子节点的优先级要低，就交换它们的位置。直到已经比较到二叉树的底层了或者当前节点优先级比左右子节点都高；


## CopyOnWriteArrayList写时复制

**概述**：CopyOnWriteArrayList 针对**读操作未加锁**（ReentrantLock），对所有的**写操作加锁**。使用的是「**写时复制**」的思想，在进行写操作时，不会直接在原数组上进行修改，而是先将原数组复制一份，在新数组上进行修改，修改完成后再将原数组的引用指向新数组。

**应用场景**：

- **读多写少的场景**：读操作性能较高，写操作性能较低，所以适用于读多写少的场景，如配置信息的存储和读取、缓存数据的管理等。
- **对数据实时性要求不高的场景**：由于读操作是基于数组快照的，存在一定的数据延迟，所以适用于对数据实时性要求不高的场景，如统计信息的展示、日志数据的读取等。
...
- **线程安全**：通过写时复制的策略，保证了在多线程环境下的线程安全，不需要使用传统的同步机制（如 `synchronized`），避免了同步带来的性能开销。
- **读操作高效**：读操作不需要加锁，多个线程可以同时进行读操作，因此读操作的性能非常高，适用于读多写少的场景。

**缺点**

- **写操作开销大**：写操作需要复制整个数组，因此写操作的性能较低，尤其是在数组元素较多的情况下，复制数组会消耗大量的内存和 CPU 资源。
- **内存占用高**：由于每次写操作都会创建一个新的数组，会导致内存占用增加，尤其是在频繁进行写操作的场景下，可能会导致内存溢出。
- **数据实时性问题**：由于读操作是基于数组快照的，所以在写操作完成后，读操作可能无法立即看到最新的数据，存在一定的数据延迟。
...
- 单链表：单链表就是维护了一个指向后继节点的指针，可以通过前一个节点找到后面的节点。查找元素的效率很低，**平均时间复杂度 O(n)**。
- **加索引**：为了提高查找的效率，可以给元素加一层索引。为了更进一步提高查找效率，可以继续给元素添加一层索引。当然不能无限制的添加层数，一般我们在实现的时候是会增加限制的。这是一种**空间换时间**的做法，具体的实现我们其实可以看成二分查找。

**操作：**

- **查找操作**：在递增顺序的跳表中进行查找时，从最高层级的头节点开始。在当前层级比较下一节点键值和目标键值：若更小，就右移继续比；若相等，查找成功；若更大，则下移一层。重复上述比较过程，直至找到目标键（查找成功），或者到达最底层末尾仍未找到（查找失败）。
  - **初始化**：从跳表的最高层级（`maxLevel`）的头节点开始。头节点不存储实际的数据，它只是作为查找的起始点。
  - 当前层级查找：在当前层级，比较当前节点的下一个节点的键值与 `targetKey`的大小。
    - **若下一个节点键值小于 `targetKey`**：由于是递增顺序，说明目标键可能在当前节点的右侧，所以沿着当前层级向右移动到下一个节点，继续进行比较。
    - **若下一个节点键值大于 `targetKey`**：表明目标键不在当前层级的右侧，此时需要向下移动到下一层级。
    - **若下一个节点键值等于 `targetKey`**：则查找成功，可根据需求返回该节点或其关联的值。
  - **层级下移**：当在某一层级无法继续向右移动（即下一个节点键值大于 `targetKey`）时，将当前节点移动到下一层级的相同位置，然后重复步骤 2。
  - 查找结束条件：
    - **查找成功**：在某一层级找到键值等于 `targetKey` 的节点。
    - **查找失败**：到达最底层链表的末尾，都没有找到键值等于 `targetKey` 的节点。


- **插入和删除操作**：插入和删除操作也需要从最高层级开始，先找到合适的位置，然后在相应的层级上进行插入或删除节点的操作。为了保证跳表的平衡性，插入和删除操作可能会随机决定是否在更高层级上添加或删除节点。

**优点**

- **高并发性能**：由于采用无锁算法，多个线程可以同时进行读写操作，避免了锁竞争，提高了并发性能。
- **有序性**：可以根据键的顺序进行遍历和查找，支持范围查询，适用于需要有序数据的场景。
- **可扩展性**：跳表的结构使得插入、删除和查找操作的时间复杂度平均为 O(logn)，并且在高并发场景下具有较好的可扩展性。

**缺点**

- **内存开销大**：跳表需要额外的指针来维护多个层级，因此相比普通的哈希表或链表，会占用更多的内存空间。
- **插入和删除操作相对复杂**：插入和删除操作需要维护跳表的结构，可能会涉及到节点的添加和删除，以及层级的调整，因此相对复杂。

**使用场景**

- **高并发有序数据存储**：当需要在多线程环境下存储有序数据，并且对并发性能有较高要求时，可以使用。例如，在分布式系统中，存储有序的配置信息或者统计数据。
- **范围查询**：当需要进行范围查询时，跳表有序性和导航方法可以方便地实现范围查找功能。例如，查找某个时间段内的日志记录。
...
- **数组 + 链表 + 红黑树**：数组作为哈希表的基础存储结构，每个数组元素称为一个桶（bucket）。当多个键值对的哈希值映射到同一个桶时，这些键值对会以链表的形式存储在该桶中。当链表长度超过一定阈值（默认为 8）且数组长度大于 64 时，链表会转换为红黑树，以提高查找效率，红黑树的查找、插入和删除操作的时间复杂度为 *O*(log*n*)。
- **CAS + synchronized**：采用 `CAS`和 `synchronized` 来保证并发操作的线程安全。`CAS` 是一种无锁算法，用于在更新操作时比较内存中的值和预期值，如果相等则进行更新，否则重试，这种方式避免了锁的开销，提高了并发性能。`synchronized` 用于对链表或红黑树进行加锁，保证在对这些结构进行修改时的线程安全。
- **数组长度**：数组一定是 2 的 n 次幂。一个原因是可以使用位运算高效的计算索引值，让 key 均匀分布，减少哈希冲突；另一个原因就是扩容迁移的便捷及高效，可以实现多线程并发协作完成数据迁移；
...
1. **计算哈希值**：首先计算键的哈希值，根据哈希值找到对应的桶位置。
2. **判断数组是否初始化**：如果数组还未初始化，则使用 `CAS` 操作进行初始化。
3. **判断桶是否为空**：如果桶为空，使用 `CAS` 操作将新节点插入到该桶中。
4. **判断是否正在扩容**：如果桶正在进行扩容操作，则当前线程会协助进行扩容。
5. **加锁插入**：如果桶不为空且没有正在扩容，则使用 `synchronized` 对该桶进行加锁，然后遍历链表或红黑树，查找是否存在相同的键。如果存在，则更新其值；如果不存在，则将新节点插入到链表或红黑树中。插入后，如果链表长度超过阈值，则将链表转换为红黑树。
