...
- **多线程并行回收**：默认情况下，它开启的回收线程数量和处理器核心数一致。若机器核心数量众多，可通过 `-XX:ParallelGCThreads` 参数对垃圾回收线程数加以限制。
...
// 自定义类加载器，打破双亲委派模型
class CustomWebappClassLoader extends ClassLoader {
    private String classPath;

    public CustomWebappClassLoader(String classPath, ClassLoader parent) {
        super(parent);
